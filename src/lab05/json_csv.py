...
def json_to_csv(json_path: str, csv_path: str) -> None:
    ...
    all_fields = list(
        data[0].keys()
    )  # берем ключи первого объекта как стартовый набор полей

    for item in data[
        1:
    ]:  # проходим остальные объекты и добавляем недостающие поля в all_fields
...
try:
    json_to_csv(
        "data/lab05/samples/empty_json.json", "data/lab05/out/empty_from_json.csv"
    )
    csv_to_json(
        "data/lab05/samples/no_header_csv.csv", "data/lab05/out/no_header_from_csv.json"
    )
    json_to_csv(
        "data/lab05/samples/test_missing_fields.json",
        "data/lab05/out/missing_fields_from_json.csv",
    )
